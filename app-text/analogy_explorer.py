...
def graph_isomorphism_experiment(G1, G2, graph_size=20, prob=0.10, deletions=0):
    ...
    show_FDG = True
    if show_FDG:
        output_filename_tgt = ShowGraphs.show_blended_space(G1_preds, [], [], \
                                                            "Expt " + str(G1.number_of_nodes()) + " " + str(
                                                                G1.number_of_edges()) + " " + str(grf_id) + " Tgt")
        output_filename_src = ShowGraphs.show_blended_space(G2_preds, [], [], \
                                                            "Expt " + str(G1.number_of_nodes()) + " " + str(
                                                                G1.number_of_edges()) + " " + str(grf_id) + " Src")
    ...
    if show_FDG:
        output_filename_hogs = ShowGraphs.show_blended_space_big_nodes(G1, generic_preds, unmapped_G1_edges,
                                                                       unmapped_G2_edges, \
                                                                       "Expt " + str(G1.number_of_nodes()) + " " + str(
                                                                           G1.number_of_edges()) + " " + str(
                                                                           grf_id) + " HOGS")
    ...
    print("Similarity: ", round(len(list_of_mapped_preds_1) / max_map_edges * 100, 2), "% ")
    return output_filename_tgt, output_filename_src, output_filename_hogs
    # ############################################# VF++ #############################################
    time1 = time.time()
...
def run_single_graph_matching_test(G1, G2):
    # set the parameters, select a middle number
    ...
    output_filename_tgt, output_filename_src, output_filename_hogs = graph_isomorphism_experiment(G1, G2,
                                                                                                  graph_size=num_of_nodes,
                                                                                                  prob=prob,
                                                                                                  deletions=dels)
...
def remove_files():
    time.sleep(20)
    for file_path in [output_filename_tgt, output_filename_src]:
        if os.path.exists(file_path):
            os.remove(file_path)


####################################Streamlit####################################
st.title('Analogy Explorer')
...
if st.button("Analyze"):
    if text1 and text2:
        ...
        triples2 = process_document(text2)
        ...
        with contextlib.redirect_stdout(f):
            output_filename_tgt, output_filename_src, output_filename_hogs = run_single_graph_matching_test(G1, G2)
        output = f.getvalue()

        st.write("Analysis Results:")
        st.text(output)

        # get the base64 content of the files
        b64_tgt = get_file_content_as_base64(output_filename_tgt)
        b64_src = get_file_content_as_base64(output_filename_src)
        b64_hogs = get_file_content_as_base64(output_filename_hogs)
        ...
        <script>
            window.onload = function() {{
                ...
                downloadFile("target_result.html", "{b64_tgt}");
                downloadFile("source_result.html", "{b64_src}");
                downloadFile("HOGS_result.html", "{b64_hogs}");
            }}
        </script>
        """
        ...
        st.success("Analysis complete. The result HTML files should have downloaded automatically.")
    else:
